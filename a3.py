...
def title_by_year(matches: List[str]) -> List[str]:
    ...
    #         matches.append(get_title(movie))
...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    return [get_title(movie) for movie in movie_db if startYear <= get_year(movie) <= endYear]
...
def title_by_director(matches: List[str]) -> List[str]:
    ...
    """
    director = matches[0]
    return [get_title(movie) for movie in movie_db if get_director(movie) == director]
...
def year_by_title(matches: List[str]) -> List[int]:
    ...
    return [get_year(movie) for movie in movie_db if get_title(movie) == title]
